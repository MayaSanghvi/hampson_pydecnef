function [ww, ix_eff_all, errTable_tr, errTable_te, parm, AXall,Ptr,Pte,t_train_est,t_test_est, tr_all_iter_acc, te_all_iter_acc, all_ww, n_feat_per_iter] =...
    decnef_biclsfy_islrvar(x_train, t_train, x_test, t_test, niter, varargin)
...
% tr_all_iter_acc : train accuracy for each iteration
% te_all_iter_acc : test accuracy for each iteration
% all_ww          :  weights vector for each iteration
% n_feat_per_iter : # of used feature per iteration
%
% --- Reference
...
% 11/2023 : Modified by Hugo SIX, ATR CNS, Decnef
...
if norm_sep == 0
    ...
    [x_train, ~, ~] = normalize_feature(x_train, parm.scale_mode, parm.mean_mode);
    [x_test, ~, ~] = normalize_feature(x_test, parm.scale_mode, parm.mean_mode);
end
...
tr_all_iter_acc = nan(1, niter);
te_all_iter_acc = nan(1, niter);

all_ww = zeros(size(Xtr,2), niter);
AXall = cell(1,niter);
n_feat_per_iter = nan(1, niter);
for nn = 1 : niter

    fprintf('iteration %d .... \n', nn);
    Xtr_recy = Xtr(:,ix_recy);

    [tmp_ww, tmp_jx_eff, ~, tmp_AXall] = slr_learning_var2(b_train, Xtr_recy,...
    ...
    % DECNEF CODE
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    all_ww(ix_recy,nn) = tmp_ww;
    n_feat_per_iter(nn) = length(tmp_jx_eff);
    % compute accuracies for each iterations
    [tr_iter_label_est, ~] = calc_label(Xtr, [zeros(Nfeat,1) ww]);
    errTable_tr_iter = slr_error_table(t_train, tr_iter_label_est, unique([t_train;t_test]));
    tr_all_iter_acc(nn) = calc_percor(errTable_tr_iter);

    [iter_label_est, ~] = calc_label(Xte, [zeros(Nfeat,1) ww]);
    errTable_te_iter = slr_error_table(t_test, iter_label_est, unique([t_train;t_test]));
    te_all_iter_acc(nn) = calc_percor(errTable_te_iter);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
