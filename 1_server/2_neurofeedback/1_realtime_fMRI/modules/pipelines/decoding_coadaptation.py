...
Decoder coadaptation for real-time neurofeedback.

Adaptively updates the decoder based on incoming volumes that meet
a prediction confidence threshold, allowing the model to improve
over the course of a neurofeedback session.

Steps:
    1. Load existing coadaptation data (if available) or initialize from training data
    2. Evaluate each incoming volume against the coadaptation acceptance criteria
    3. Add qualifying volumes to the training dataset
    4. Retrain the decoder on the expanded dataset
...
def coadaptation(vol_list, ground_truth, model_file):
    ...
    for vol in vol_list:
        generated_probs = no_coadapted_model.predict_proba(vol)
        decoding_prob = generated_probs[0][int(ground_truth)]
    ...
    # Build a new classifier pipeline matching the training configuration
    if classification_method == "svm":
        pipeline = make_pipeline(LinearSVC(dual=True, C=1, class_weight='balanced', random_state=12345))
    elif classification_method == "svmlinear":
        pipeline = make_pipeline(LinearSVC(penalty='l1', dual=False, C=1, class_weight='balanced', random_state=12345))
